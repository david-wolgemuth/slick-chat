...
{
  if (userId) {
    return this.session.users.find(user => user._id === userId) || null;
  }
  if (teamId) {
    return this.session.users.find(user => user.team === teamId) || null;
  }
};
...
{
   if (userId) {
    this.session.users = this.session.users.filter(user => user._id !== userId);
  }
  if (teamId) {
    this.session.users = this.session.users.filter(user => user.team !== teamId);
  } 
};
